...
import {image_list_type} from "../../../types/types";
...
export default async function search({searchParams}: {
    ...
    const error = "Looks like that got lost in the space 🤣 Try checking your internet maybe.";
    const {q} = searchParams;
    const [imageList, setImageList] = useState({});
    const [searchTerm, setSearchTerm] = useState("Jupiter");
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (topic.current && topic.current.value == "") {
            ...
            if (topic.current) {
                ...
                setSearchTerm(topic.current.value)
            }
        }
    };
        ...
        {" "}
          Go back
            ...
            <div className="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
                {imageList?.data?.collection?.items?.length === 0 ? (
                        <div className="flex flex-col justify-center items-center">
                            <img src="/empty_state.gif" className="block mx-auto" alt="Loading..."/>
                            <h1 className='text-center text-md-left p-3'>Woah, looks like that thing doesn't exist in the
                                Universe yet 😂</h1>
                        </div>)
                    : (
                        <div className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-10">
                            {
                                imageList?.data?.collection?.items?.map((image: image_list_type, index: any) => {
                                    ...
                                    return (
                                        <DateImage
                                            ...
                                            src={image.links[0].href} alt={"Some"}></DateImage>
                                    );
                                })}
                        </div>)}
                {/* starts here */}
